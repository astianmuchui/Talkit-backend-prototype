<?php
    //Get all the chat rooms that the user is involved in
    class MEMBER_CHATROOMS {        
        ...
        public function get_member_chatrooms(){
            ...
            foreach($rs as $r){
                ...
                if(strpos($r_name,$this->logged_in_user)!== false){
                    $tnm = $r['tname'];
                    ...
                    if($ltst_msg['id_1'] == base64_encode($this->logged_in_user)){
                        ...
                        $div = '<div class="chat">
                        <a href="./action/create_room.php?id='.$b64.'">'.$DB_PARSE_DECODE.'</a>
                        ...
                        $DB_PARSE_DECODE = base64_decode($DB_PARSE['uname']);
                        $div = '<div class="chat">
                        <a href="./action/create_room.php?id='.$b64crypt.'">'.$DB_PARSE_DECODE.'</a>
                        <small>'.$decry_hex_mess.'</small>
                        <small>'.substr($ltst_msg['date'],11,5).'</small>
                         </div>';
